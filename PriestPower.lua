...
FiveManBlessingIcon[3] = "Interface\\Icons\\Spell_Holy_PrayerOfHealing02";
FiveManBlessingIcon[4] = "Interface\\Icons\\Spell_Magic_MageArmor";
FiveManBlessingIcon[5] = "Interface\\Icons\\Spell_Nature_LightningShield";
...
function PallyPowerBuffButton_OnClick(btn, mousebtn)
    ...
    if LastCast[btn.buffID .. btn.classID] and LastCast[btn.buffID .. btn.classID] > (3 * 60) - 30 then
        RecentCast = true
    end
    for unit, stats in CurrentBuffs[btn.classID] do
        if SpellCanTargetUnit(unit) and not (RecentCast and string.find(table.concat(LastCastOn[btn.classID], " "), unit)) then
            ...
            PP_NextScan = 1
            LastCast[btn.buffID .. btn.classID] = 3 * 60;
